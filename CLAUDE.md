...
### 8.26 Mobile responsiveness pass (DONE, build green)

The desktop layout was polished but mobile (≈390px phones) had layout problems. Fixed **format only, no copy or content changes**; every change is gated behind `sm:`/`md:` (or is mobile-base only) so the desktop layout is byte-for-byte unchanged. Fixes:
- **Nav (`Nav.jsx`)** — was one flex row with the wordmark + 5 links and no mobile handling, so the wordmark wrapped and the links overflowed ("OOO" clipped). Now: wordmark is `shrink-0 whitespace-nowrap`; the links `<ul>` is a **compact single row that scrolls sideways if needed** (`no-scrollbar flex min-w-0 items-center gap-3 overflow-x-auto whitespace-nowrap text-sm sm:gap-5 md:gap-7`, each `<li>` `shrink-0`); nav padding tightened on mobile (`px-4 sm:px-6 md:px-8`). At 390px all five links now fit on one line; narrower phones scroll. (Chosen approach: "compact scrolling row", not a hamburger.)
- **`no-scrollbar` utility** added to `src/index.css` (`@layer utilities`) — hides the scrollbar while keeping the nav row scrollable.
- **Landing name-card bullets (`Landing.jsx`)** — the four bullets had `whitespace-nowrap` (intentional on desktop, one line each beside the headshot) which **clipped on phones**. Changed to `sm:whitespace-nowrap` so they wrap below `sm`.
- **Landing contact strip (`Landing.jsx`)** — long email + wide `gap-x-8` overflowed. Now `flex-col ... sm:flex-row sm:flex-wrap` (stacks and centers on mobile, unchanged inline row on `sm+`) with `break-words`.
- **Landing hero ↔ name-card overlap (`Landing.jsx`)** — the card pull-up was `-mt-[120px]`, too aggressive on the short mobile skyline band (covered the headline). Mobile base changed to `-mt-12`; `sm:-mt-[136px] lg:-mt-40` (desktop) untouched.
- **Resume degree line (`Resume.jsx`)** — `whitespace-nowrap` on the degree name clipped on phones; now `sm:whitespace-nowrap`.
- **Landing hero, mobile (`Landing.jsx`)** — the uncropped natural skyline renders as a short band on a phone, so the headline "I tell stories with data" and the name card were crammed together (card covered the headline). The hero image is now **two responsive variants**: `md:hidden` shows a **taller cover crop** (`SmartImage` non-natural, `ratio="h-[56vh]"`, `fit="cover"`, `position="object-center"`) so the skyline reaches mid-screen and leaves room below the headline; `hidden md:block` keeps the **exact original natural uncropped photo** on desktop (unchanged). With the taller mobile hero the name card's mobile pull-up was then tuned to `-mt-40` (base) so the card tucks right up against the bottom of the visible skyline for a cohesive join (headline stays fully clear above it); `sm:-mt-[136px] lg:-mt-40` (desktop) unchanged. Also tightened the mobile gap below the card: the contact strip is `mb-2` on mobile (`md:mb-11` unchanged) and the "little story about me" `Section` got `innerClassName="pt-10 md:pt-20"` to cut its mobile top padding.
- **Fun Facts hero, mobile (`FunFacts.jsx`)** — the hero used `aspect-[32/9]` (a cinematic wide band ≈110px tall on a phone), which squeezed the overlaid eyebrow + title against the transparent nav. Ratio is now responsive: `aspect-[4/3] sm:aspect-[16/7] md:aspect-[32/9]` (taller on mobile, **desktop 32/9 unchanged**).
- **Verified:** every route measured at a true 390px CSS viewport reports `scrollWidth == innerWidth` (no page-level horizontal overflow). Remaining elements wider than the viewport are all inside intended scroll/clip containers (nav scrolling row, code blocks' `overflow-x-auto`, the Fun Facts photo carousel). The Tableau embeds already scale to container width.

**Headless-screenshot gotcha for future mobile testing:** Chrome `--headless` (new or old) **ignores `--window-size` below ~500px and clamps the CSS viewport to 500px** (Chrome enforces a min window width), so a `--window-size=390` capture renders at 500 CSS px and crops to 390, making content *look* clipped when it isn't. To capture a **true** narrow viewport, drive Chrome over CDP with `Emulation.setDeviceMetricsOverride`. There are reusable Node scripts (Node 24 has a global `WebSocket`, no deps): `/tmp/shot.mjs` (emulate width, screenshot, optionally scroll to trigger `whileInView` sections then full-page capture) and `/tmp/check.mjs` (report `scrollWidth` vs `innerWidth` + overflowing elements per route). Launch Chrome with `--remote-debugging-port=9222` first. (These live in /tmp, not the repo.)
